...
function App() {
  ...
  const renderRecommended = () => selected ? <section className="screen event-detail">
    ...
    {selected.host && <div className="event-host"><Users size={28} /><div><span className="event-host-label">Hosted by</span><strong className="event-host-name">{selected.host}</strong></div><ListenButton text={'Hosted by ' + selected.host} label="host" slow={slow} language={language} /></div>}
